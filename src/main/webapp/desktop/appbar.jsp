...
<header class="pm-appbar">
    ...
    <div class="pm-appbar-actions">
        <a href="<%= _ab_cp %>/desktop/notifications.jsp" class="pm-appbar-icon-btn">
        ...
        <a href="<%= _ab_cp %>/desktop/mypage.jsp" class="pm-appbar-avatar">
...
(function() {
    ...
    function refreshAbDot() {
        fetch(_abCp + '/notifApi?action=unreadCount', {credentials: 'same-origin'})
            ...
            .then(function(d) {
                ...
                if (!dot) return;
                dot.classList.toggle('on', (d.unreadCount || 0) > 0);
            }).catch(function() {});
    }
    refreshAbDot();
    ...
    window.pmGlobalSearch = function(q) {
        ...
        _abTimer = setTimeout(function() {
            location.href = _abCp + '/desktop/myCase.jsp?q=' + encodeURIComponent(q.trim());
        }, 300);
    };
})();
</script>
