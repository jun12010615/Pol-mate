...
if (postIdParam == null || postIdParam.trim().isEmpty()) {
    response.sendRedirect(request.getContextPath() + "/desktop/board.jsp");
    return;
}
...
<main class="pm-page">
    <a href="<%= request.getContextPath() %>/desktop/board.jsp" class="back-link">
...
function deletePost() {
    ...
    fetch(_ctx + '/board', { method: 'POST', body: fd, credentials: 'same-origin' })
        .then(function(r) { return r.json(); })
        .then(function(d) {
            if (d.success) location.href = _ctx + '/desktop/board.jsp';
            else showToast(d.error || '삭제에 실패했습니다.');
        }).catch(function() {});
}
