...
try {
    ...
    PreparedStatement _ps = _conn.prepareStatement("SELECT user_rank FROM users WHERE user_id=?");
    _ps.setString(1, loginUser);
    ResultSet _rs = _ps.executeQuery();
    if (_rs.next() && userRank.isEmpty()) userRank = _rs.getString("user_rank") != null ? _rs.getString("user_rank") : "";
    _rs.close(); _ps.close();
} catch (Exception _e) { _e.printStackTrace(); }
finally { _mgr.freeConnection(_conn); }
...
<script>var _ctx='<%= request.getContextPath() %>'; var _loginUser='<%= loginUser %>'; var _userName='<%= userName %>'; var _userRank='<%= userRank %>';</script>
<style>
...
function openPost(id) {
    location.href = _ctx + '/desktop/boardView.jsp?postId=' + id;
}
