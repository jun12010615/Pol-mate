...
int cntCase = 0, cntTranscript = 0, cntBoard = 0;
String userPhone = "", userEmail = "", userDeptName = "";
...
try {
    ...
    ResultSet rs;
    ps = conn.prepareStatement("SELECT user_phone, user_email, user_rank FROM users WHERE user_id=?");
    ps.setString(1, loginUser); rs = ps.executeQuery();
    if (rs.next()) { userPhone = rs.getString("user_phone") != null ? rs.getString("user_phone") : ""; userEmail = rs.getString("user_email") != null ? rs.getString("user_email") : ""; if (userRank.isEmpty()) userRank = rs.getString("user_rank") != null ? rs.getString("user_rank") : ""; }
    rs.close(); ps.close();
    ps = conn.prepareStatement("SELECT COUNT(*) FROM cases WHERE user_id=?");
    ps.setString(1, loginUser); rs = ps.executeQuery(); if (rs.next()) cntCase = rs.getInt(1); rs.close(); ps.close();
    ps = conn.prepareStatement("SELECT COUNT(*) FROM transcripts WHERE user_id=?");
    ps.setString(1, loginUser); rs = ps.executeQuery(); if (rs.next()) cntTranscript = rs.getInt(1); rs.close(); ps.close();
    ps = conn.prepareStatement("SELECT COUNT(*) FROM board_posts WHERE user_id=?");
    ps.setString(1, loginUser); rs = ps.executeQuery(); if (rs.next()) cntBoard = rs.getInt(1); rs.close(); ps.close();
...
<main class="pm-page">
    ...
    <div class="stat-grid">
        <div class="stat-card"><div class="stat-val"><%= cntCase %></div><div class="stat-lbl">담당 사건 수</div></div>
        <div class="stat-card"><div class="stat-val"><%= cntTranscript %></div><div class="stat-lbl">작성 진술 조서</div></div>
        <div class="stat-card"><div class="stat-val"><%= cntBoard %></div><div class="stat-lbl">커뮤니티 게시글</div></div>
    </div>
    ...
    <div class="content-grid">
        ...
        <div class="section-card">
            ...
            <div class="section-body">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value"><%= userPhone.isEmpty() ? "-" : userPhone %></span>
                </div>
                <div class="info-row">
                    <span class="info-label">이메일</span>
                    <span class="info-value"><%= userEmail.isEmpty() ? "-" : userEmail %></span>
                </div>
            </div>
        </div>
        ...
        <div>
            <div class="section-card" style="margin-bottom:16px;">
                ...
                <div class="section-body">
                    <a href="<%= request.getContextPath() %>/desktop/myCase.jsp" class="menu-item">
                    ...
                    <a href="<%= request.getContextPath() %>/desktop/writeTranscript.jsp" class="menu-item">
                    ...
                    <a href="<%= request.getContextPath() %>/desktop/board.jsp" class="menu-item">
            ...
            <div class="section-card">
                ...
                <div class="section-body">
                    ...
                    <div class="danger-zone">
                        ...
                        <form action="<%= request.getContextPath() %>/mypage" method="post" onsubmit="return confirm('로그아웃 하시겠습니까?');">
